...
private:
  ...
  private:
     // instance of Key_compare
      Key_compare compare;
  public:
      // bool operator()(Key_compare pair1, Key_compare pair2) const {
...
public:
  ...
  //       using "Value_type()".
    Iterator find(const Key_type& k) const {
        std::pair<Key_type, Value_type> pair_find = {k, Value_type()};
        // Map::find() takes in a key, but BST::find() takes in a pair of {key, value}
        return tree.find(pair_find);
    }
  ...
  // HINT: http://www.cplusplus.com/reference/map/map/operator[]/
    Value_type& operator[](const Key_type& k) {
        // use insert()
        //using Pair_type = std::pair<Key_type, Value_type>;
//        std::pair<Key_type, Value_type> map_pair = {k, Value_type()};
//The BinarySearchTree::insert will return the iterator points the newly inserted value.
//      Either (*it).second or it->second will work.
        if (find(k) == end()) {
            Pair_type map_pair(k, Value_type());
            tree.insert(map_pair);
        }
  ...
  //           the value true.
    std::pair<Iterator, bool> insert(const Pair_type &val){
        ...
        if (tree.find(val) == tree.end()) {
            pair = {tree.insert(val), true};
            return pair;
        }
        pair = {tree.find(val), false};
        return pair;
    }
