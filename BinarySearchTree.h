...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
      // use new keyword - allocating dynamic memory
      if (!node) return node;
      Node *ptr = new Node(node->datum, nullptr, nullptr);
      ptr->left = copy_nodes_impl(node->left);
      ptr->right = copy_nodes_impl(node->right);
      return ptr;
  }
...
//       use new
      if (!node) {
          Node *ptr = new Node(item, nullptr, nullptr);
          return ptr;
      }
     // “Compare” is the type of a functor that should compare two elements and
      // return true if the first is smaller.
      // node datum less than item, item greater
      if (less(node->datum, item)) {
          node->right = insert_impl(node->right, item, less);
      }
      // node datum greater than item, item smaller
      if (less(item, node->datum)) {
          node->left = insert_impl(node->left, item, less);
      }
          // Note that the spec for insert_impl asks you to return the new/existing root
      // of the tree, not the node which represents the inserted element.
      return node;
  }
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
      // fxn for bst that returns some property tht would ensure an entire tree is
      // < a certain value
      if (!node) return true;
      else if (!node->left && !node->right) return true;
      else if (less(max_element_impl(node->left)->datum, node->datum) &&
               less(node->datum, min_element_impl(node->right)->datum)) return true;
      else {
